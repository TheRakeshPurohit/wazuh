...
async def test_restore_default_passwords(forward_mock, safe_load_mock, print_mock, user_input):
    ...
    with patch("getpass.getpass", return_value=user_input):
        ...
        if user_input != "":
            forward_mock.assert_called_with(update_user, f_kwargs={'user_id': '1', 'password': user_input},
...
@patch("builtins.print")
@patch("scripts.security_resources.cluster_utils.forward_function")
async def test_reset_rbac_database(forward_mock, print_mock, user_input):
    """Check if the `restore_default_passwords` uses the correct parameters when called.
    ...
    with patch("builtins.input", return_value=user_input):
        if user_input == "RESET":
            ...
            forward_mock.assert_called_with(rbac_db_factory_reset, request_type="local_master")
